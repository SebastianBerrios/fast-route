...
**Pre-flight before every SDD/Judgment-Day Agent call (mandatory):**

1. Identify the SDD/Judgment-Day phase key (`sdd-apply`, `sdd-verify`, `jd-judge-a`, etc.) or use `default` only as a phase fallback.
2. Look up the alias in the Model Assignments table.
3. Include `model: "<alias>"` in the SDD/Judgment-Day Agent tool call.
4. For generic/non-SDD delegation, do not use this table and omit `model` unless the user explicitly requested an override.
